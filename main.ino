...
vin_pwr = vin_bus * (vin_curr / 1000.0f); // Perubahan: daya dihitung manual dari V x I
...
vout_pwr  = vout_bus * (vout_curr / 1000.0f); // Perubahan: daya dihitung manual juga
...
lcd.setCursor(0, 1);
lcd.printf("Pin:%.2fmW", vin_pwr * 1000); // Tambahan baris tampilan daya input

lcd.setCursor(0, 2);
lcd.printf("Out:%.2fV %.1fmA", vout_bus, vout_curr);
lcd.setCursor(0, 3);
lcd.printf("Pout:%.2fmW", vout_pwr * 1000); // Tambahan baris tampilan daya output
