...
#include <math.h>  // <-- Tambahan untuk fungsi fabs()
...
float ina219_get_current(TwoWire &wire, uint8_t addr, float current_LSB) {
  ...
  return fabs(val * current_LSB * 1000.0f); // mA, pakai fabs() supaya selalu positif
}
...
void readTask(void *pvParameters) {
  for (;;) {
    ...
    vin_pwr = vin_bus * (vin_curr / 1000.0f); // Daya = V x I
